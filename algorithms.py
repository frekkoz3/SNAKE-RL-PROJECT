...
class RLAlgorithm:
    ...
    def get_action_epsilon_greedy(self, s, eps, possible_actions = None):
        ...
        """
        complete_subkey(self.Qvalues, s, default=[i for i in possible_actions])
        ran = np.random.rand()
        
        if (ran < eps):
            prob_actions = np.ones(len(possible_actions))/len(possible_actions)
        else:
            prob_actions = np.zeros(len(possible_actions))
            best_action = argmax_over_dict_given_subkey_and_possible_action(self.Qvalues, (*s, ), possible_actions, default=[i for i in possible_actions])[2]
            best_idx = possible_actions.index(best_action)
            prob_actions[best_idx] = 1
            
        # take one action from the array of actions with the probabilities as defined above.
        a = np.random.choice(possible_actions, p=prob_actions)
        return a 
        
    def get_action_greedy(self, s, possible_actions = None):
        if possible_actions is None:
            ...
            a = argmax_over_dict_given_subkey_and_possible_action(self.Qvalues, (*s, ), possible_actions, default=[i for i in range (self.action_space)])[2]
        return a
    ...
    def play(self, env, bracketer):
        ...
        while not done and keep:
            possible_actions = env.get_possible_actions(last_action)
...
class DeepDoubleQLearning(RLAlgorithm):
    ...
    def get_action_epsilon_greedy(self, state, eps, possible_actions=None):
        """
        Uses an epsilon-greedy policy to choose an action based on the current state.
        """
        if np.random.rand() < eps:
            return np.random.choice(possible_actions)
        else:
            # Assicurati che state sia un torch tensor 1D
            s = torch.tensor(state, dtype=torch.float32, device=self.device).unsqueeze(0)
            with torch.no_grad():
                qvals = self.dqn_online(s)           # shape [1, action_dim]
            return qvals.argmax(dim=1).item()         # l’azione con Q più alta


    def get_action_greedy(self, state, possible_action=None):
        ...
        if possible_action is None or best in possible_action:
            return best
        # altrimenti scegli il massimo tra quelli permessi
        sub_q = qvals[0, possible_action]
        return possible_action[sub_q.argmax().item()]
